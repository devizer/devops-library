...
1. End-to-end and integration testing of .NET 1.0 ... 10.0+ applications.
...
By default, the command skips dynamic linker configuration if OpenSSL libraries are already registered. The --force option overrides this behavior.
...
To distribute it including OpenSSL 3.5 LTS as part of the application side-by-side:
```
runtimes="arm arm64 x64 musl-arm musl-arm64 musl-x64"
for rid in $runtimes; do
  Run-Remote-Script https://devizer.github.io/devops-library/install-libssl.sh \
      v3.5 \
      --target-folder "./superapp-linux-$rid/libssl-v3.5" \
      --rid "linux-$rid"
done 
```
